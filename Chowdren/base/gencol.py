...
def write_init(writer, name, col, x, y):
    if name == 'box':
        return (BoxCase(col, x, y),)
    if name == 'tsprite':
    ...
    if name in has_sprite:
        ...
        if name == 'tsprite':
            return (BoxTransformCase(col, x, y),
                    ImageTransformCase(col, x, y),
                    AlphaTransformCase(col, x, y))

        return (ImageCase(col, x, y), AlphaCase(col, x, y))
...
def write_func(writer, name1, type1, name2, type2):
    args = []
    ...
    cases_1 = write_init(writer, name1, 'a', 'offx1', 'offy1')
    cases_2 = write_init(writer, name2, 'b', 'offx2', 'offy2')
...
def main():
    ...
    writer.putln('// Generated by gencol.py, do not modify!')
    writer.putln('')
    for (name1, type1) in types:
        for (name2, type2) in types:
            write_func(writer, name1, type1, name2, type2)
