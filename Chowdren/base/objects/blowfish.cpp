...
// retrieved from https://github.com/h2so5/Blowfish
...
{
    memcpy(pary_, initial_pary, sizeof(initial_pary));
    memcpy(sbox_, initial_sbox, sizeof(initial_sbox));
...
void Blowfish::encrypt(char* dst, const char* src, size_t byte_length) const
{
    ...
    for (unsigned int i = 0; i < byte_length / sizeof(uint64_t); ++i)
    {
...
void Blowfish::encrypt_block(uint32_t *left, uint32_t *right) const
{
    for (int i = 0; i < 16; ++i)
    {
        *left  ^= pary_[i];
...
void Blowfish::decrypt_block(uint32_t *left, uint32_t *right) const
{
    for (int i = 0; i < 16; ++i)
    {
        *left  ^= pary_[17 - i];
