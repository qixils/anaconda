...
template <typename GRID> Position Searcher<GRID>::jumpD(Position p, int dx, int dy)
{
    ...
    while(true)
    {
        ...
        if ((grid(x + dx, y + dy) && grid(x, y + dy) && !grid(x + dx, y)) ||
            (grid(x + dx, y + dy) && grid(x + dx, y) && !grid(x, y + dy)))
            break;
...
template <typename GRID> inline Position Searcher<GRID>::jumpX(Position p, int dx)
{
    ...
    unsigned a = ~((!!grid(p.x - dx, y+skip)) | ((!!grid(p.x - dx, y-skip)) << 1));

    while(true)
    {
        const unsigned xx = p.x;
...
template <typename GRID> inline Position Searcher<GRID>::jumpY(Position p, int dy)
{
    ...
    unsigned a = ~((!!grid(x+skip, p.y - dy)) | ((!!grid(x-skip, p.y - dy)) << 1));

    while(true)
    {
        const unsigned yy = p.y;
...
template <typename GRID> Position Searcher<GRID>::jumpPRec(const Position& p, const Position& src) const
{
    ...
    {
        if ((grid(x, y + 1) && !grid(x - dx, y + 1)) ||
            (grid(x, y - 1) && !grid(x - dx, y - 1)))
            return p;
    }
    else if(dy)
    {
        if ((grid(x + 1, y) && !grid(x + 1, y - dy)) ||
            (grid(x - 1, y) && !grid(x - 1, y - dy)))
            return p;
    }
...
#ifdef JPS_VERIFY
        Position jpp = jumpPRec(buf[i], n->pos);
        JPS_ASSERT(jp == jpp);
#endif
        if(!jp.isValid())
            continue;
