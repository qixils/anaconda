...
b2BroadPhase::b2BroadPhase(const b2AABB& worldAABB, b2PairCallback* callback, b2Settings* set)
{
	settings = set;

	m_proxyPool = new b2Proxy[set->b2_maxProxies];
	m_bounds[0] = new b2Bound[2*set->b2_maxProxies];
	m_bounds[1] = new b2Bound[2*set->b2_maxProxies];
	m_queryResults = new uint16[set->b2_maxProxies];
	m_querySortKeys = new float32[set->b2_maxProxies];

	m_pairManager.Initialize(this, callback, set);
	...
	for (uint16 i = 0; i < settings->b2_maxProxies - 1; ++i)
	{
	...
	m_proxyPool[settings->b2_maxProxies-1].SetNext(b2_nullProxy);
	m_proxyPool[settings->b2_maxProxies-1].timeStamp = 0;
	m_proxyPool[settings->b2_maxProxies-1].overlapCount = b2_invalid;
	m_proxyPool[settings->b2_maxProxies-1].userData = NULL;
	m_freeProxy = 0;
...
void b2BroadPhase::IncrementTimeStamp()
{
	...
	{
		for (uint16 i = 0; i < settings->b2_maxProxies; ++i)
		{
			m_proxyPool[i].timeStamp = 0;
		}
		m_timeStamp = 1;
...
void b2BroadPhase::IncrementOverlapCount(int32 proxyId)
{
	...
	else
	{
		...
		b2Assert(m_queryResultCount < settings->b2_maxProxies);
...
bool b2BroadPhase::CheckValidPair(uint32 proxyId1, uint32 proxyId2)
{
	b2Shape * s1 = (b2Shape*)m_proxyPool[proxyId1].userData;
	b2Shape * s2 = (b2Shape*)m_proxyPool[proxyId2].userData;

	if(s1->GetBody() == s2->GetBody()) return false;
	if(s1->GetBody()->IsStatic() && s2->GetBody()->IsStatic()) return false;

	b2ContactFilter* cf = s1->GetBody()->GetWorld()->m_contactFilter;

	if(cf)
	{
		return cf->ShouldCollide(s1,s2);
	}
	return true;
}
...
{
	b2Assert(m_proxyCount < settings->b2_maxProxies);
	...
	{
		b2Assert(m_queryResults[i] < settings->b2_maxProxies);
		b2Assert(m_proxyPool[m_queryResults[i]].IsValid());

		if(!CheckValidPair(proxyId, m_queryResults[i])) continue;
...
{
	b2Assert(0 < m_proxyCount && m_proxyCount <= settings->b2_maxProxies);
	b2Proxy* proxy = m_proxyPool + proxyId;
...
{
	if (proxyId == b2_nullProxy || settings->b2_maxProxies <= proxyId)
	{
		b2Assert(false);
		return;
	}
	...
	for (int32 axis = 0; axis < 2; ++axis)
	{
		...
		if (deltaLower < 0)
		{
			...
			while (index > 0 && lowerValue < bounds[index-1].value)
			{
				...
				{
					if (TestOverlap(newValues, prevProxy) && CheckValidPair(proxyId, prevProxyId))
					{
		...
		if (deltaUpper > 0)
		{
			...
			while (index < boundCount-1 && bounds[index+1].value <= upperValue)
			{
				...
				{
					if (TestOverlap(newValues, nextProxy) && CheckValidPair(proxyId, nextProxyId))
					{
