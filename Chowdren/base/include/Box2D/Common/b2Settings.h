...
#ifndef RUN_ONLY
#define b2Assert(A) if(!(A)){if(AssertFail(#A, __FILE__, __LINE__))exit(0);}
#define b2Assert2(A,M) if(!(A)){if(AssertFail2(M))exit(0);}
#else
#define b2Assert(A) {}
#define b2Assert2(A,M) {}
#endif
...
struct b2Settings
{
	b2Settings()
	{
		...
		b2_angularSlop = 2.0f / 180.0f * b2_pi;
		b2_toiSlop = 8.0f * b2_linearSlop;
		b2_maxTOIContactsPerIsland = 32;
		b2_maxTOIJointsPerIsland = 32;
		b2_velocityThreshold = 1.0f;
		b2_maxLinearCorrection = 0.2f;
		b2_maxAngularCorrection = 8.0f / 180.0f * b2_pi;
		b2_maxLinearVelocity = 200.0f;
		b2_maxLinearVelocitySquared = b2_maxLinearVelocity * b2_maxLinearVelocity;
		b2_maxAngularVelocity = 250.0f;
		b2_maxAngularVelocitySquared = b2_maxAngularVelocity * b2_maxAngularVelocity;
		b2_contactBaumgarte = 0.2f;
		b2_timeToSleep = 0.5f;
		b2_linearSleepTolerance = 0.01f;
		b2_angularSleepTolerance = 2.0f / 180.0f * b2_pi;
		b2_tableCapacity = b2_maxPairs;
		b2_tableMask = b2_tableCapacity - 1;
	}
	int32 b2_maxPolygonVertices;
	int32 b2_maxProxies;
	int32 b2_maxPairs;
	float32 b2_linearSlop;
	float32 b2_angularSlop;
	float32 b2_toiSlop;
	int32 b2_maxTOIContactsPerIsland;
	int32 b2_maxTOIJointsPerIsland;
	float32 b2_velocityThreshold;
	float32 b2_maxLinearCorrection;
	float32 b2_maxAngularCorrection;
	float32 b2_maxLinearVelocity;
	float32 b2_maxLinearVelocitySquared;
	float32 b2_maxAngularVelocity;
	float32 b2_maxAngularVelocitySquared;
	float32 b2_contactBaumgarte;
	float32 b2_timeToSleep;
	float32 b2_linearSleepTolerance;
	float32 b2_angularSleepTolerance;
	int32 b2_tableCapacity;
	int32 b2_tableMask;
};
