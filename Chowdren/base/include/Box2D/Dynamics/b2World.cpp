...
b2World::b2World(const b2AABB& worldAABB, const b2Vec2& gravity, bool doSleep, b2Settings* set)
{
	settings = set;
	...
	m_broadPhase = new (mem) b2BroadPhase(worldAABB, &m_contactManager, set);
...
b2Joint* b2World::CreateJoint(const b2JointDef* def)
{
	...
	b2Joint* j = b2Joint::Create(def, &m_blockAllocator, settings);

	// Connect to the world list.
	j->m_prev = m_lastJoint;
	j->m_next = NULL;
	if (m_lastJoint)
	{
		m_lastJoint->m_next = j;
	}
	if(!m_jointList)
	{
		m_jointList = j;
	...
	j->m_node1.prev = j->m_body1->m_lastJoint;
	j->m_node1.next = NULL;
	if (j->m_body1->m_lastJoint) j->m_body1->m_lastJoint->next = &j->m_node1;
	if (!j->m_body1->m_jointList) j->m_body1->m_jointList = &j->m_node1;
	j->m_body1->m_lastJoint = &j->m_node1;

	j->m_node2.joint = j;
	j->m_node2.other = j->m_body1;
	j->m_node2.prev = j->m_body2->m_lastJoint;
	j->m_node2.next = NULL;
	if (j->m_body2->m_lastJoint) j->m_body2->m_lastJoint->next = &j->m_node2;
	if (!j->m_body2->m_jointList) j->m_body2->m_jointList = &j->m_node2;
	j->m_body2->m_lastJoint = &j->m_node2;
...
void b2World::DestroyJoint(b2Joint* j)
{
	...
	if (&j->m_node1 == body1->m_lastJoint)
	{
		body1->m_lastJoint = j->m_node1.prev;
	}
	...
	if (&j->m_node2 == body2->m_lastJoint)
	{
		body2->m_lastJoint = j->m_node2.prev;
	}
...
void b2World::Solve(const b2TimeStep& step)
{
	...
	b2Island island(m_bodyCount, m_contactCount, m_jointCount, &m_stackAllocator, m_contactListener, settings);
...
void b2World::SolveTOI(const b2TimeStep& step)
{
	...
	b2Island island(m_bodyCount, settings->b2_maxTOIContactsPerIsland, settings->b2_maxTOIJointsPerIsland, &m_stackAllocator, m_contactListener, settings);
	...
	for (;;)
	{
		...
		for (b2Contact* c = m_contactList; c; c = c->m_next)
		{
			...
			else
			{
				...
				toi = b2TimeOfImpact(c->m_shape1, b1->m_sweep, c->m_shape2, b2->m_sweep, settings);
		...
		//If the contact was set nonsolid in the update by user action
		//Then no need to deal with the reaction
		if(minContact->m_flags & b2Contact::e_nonSolidFlag)
		{
			continue;
		}
	...
	case e_circleShape:
		{
			...
			if (core)
			{
				m_debugDraw->DrawCircle(center, radius - settings->b2_toiSlop, coreColor);
			}
		}
		break;
	...
	case e_polygonShape:
		{
			...
			b2Assert(vertexCount <= settings->b2_maxPolygonVertices);
...
void b2World::DrawDebugData()
{
	...
	if (flags & b2DebugDraw::e_pairBit)
	{
		...
		for (int32 i = 0; i < settings->b2_tableCapacity; ++i)
		{
	...
	if (flags & b2DebugDraw::e_aabbBit)
	{
		...
		for (int32 i = 0; i < settings->b2_maxProxies; ++i)
		{
...
float32 b2World::RaycastSortKey(void* data)
{
	...
	if(world->m_contactFilter && !world->m_contactFilter->RayCollide((rayUserData*)world->m_raycastUserData,shape))
		return -1;
